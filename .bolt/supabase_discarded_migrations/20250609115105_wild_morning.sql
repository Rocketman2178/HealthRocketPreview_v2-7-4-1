/*
  # Fix Challenge Actions Calculation

  1. Updates
    - Improves the challenge_actions calculation in the calculate_daily_user_insights function
    - Adds a more comprehensive query to count verification posts in challenges
    - Preserves all other working functionality
  
  2. Recalculation
    - Recalculates data for June 6-8, 2025 to update challenge_actions counts
*/

-- Create or replace the function with improved challenge_actions calculation
...
BEGIN
  ...
  -- Calculate new users created on this date
  ...
  -- IMPROVED: Calculate challenge actions for this date with a more comprehensive query
  -- This counts all verification posts made in any challenge chat on this date
  SELECT COUNT(*) INTO v_challenge_actions
  FROM chat_messages cm
  WHERE DATE(cm.created_at) = p_date
    AND cm.is_verification = true
    AND cm.chat_id LIKE 'c\_%'; -- Match any chat ID that starts with 'c_'
  ...
  -- Calculate health assessments completed on this date
  ...
  ELSE
    -- Update existing record (without updated_at column)
...
BEGIN
  ...
  -- Also calculate for today to ensure current data is updated
...
VALUES (
  'migration_fix_challenge_actions', 
  'completed', 
  'Fixed challenge_actions calculation in all_user_insights and recalculated data'
)
