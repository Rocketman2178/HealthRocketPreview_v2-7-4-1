/*
  ...
  1. New Functions
    ...
    - Adds a custom approach to insert into completed_challenges
    - Fixes the sync_fp_earnings trigger function to handle missing quest_id
...
-- First fix the sync_fp_earnings function to handle missing quest_id
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'completed_quests' THEN
    v_item_type := 'quest';
    v_item_name := 'Quest: ' || NEW.quest_id;
    v_fp_amount := NEW.fp_earned;
    
    -- Get health category from quest library
    SELECT category INTO v_health_category
    FROM quest_library
  ...
  ELSIF TG_TABLE_NAME = 'health_assessments' THEN
    v_item_type := 'health_assessment';
    v_item_name := 'Health Assessment';
    v_fp_amount := 10; -- Default FP for health assessment
  ...
  IF v_fp_amount > 0 THEN
    ...
    INSERT INTO fp_earnings (
      ...
      CASE 
        ...
        WHEN TG_TABLE_NAME = 'completed_quests' AND TG_OP = 'INSERT' THEN 
          CASE 
            WHEN EXISTS (SELECT 1 FROM information_schema.columns 
                         WHERE table_schema = 'public' 
                         AND table_name = 'completed_quests' 
                         AND column_name = 'quest_id')
            THEN NEW.quest_id
            ELSE NEW.id::text
          END
        WHEN TG_TABLE_NAME = 'health_assessments' THEN NEW.id::text
        ELSE NEW.id::text
      END,
      v_item_name,
      v_item_type,
...
GRANT EXECUTE ON FUNCTION public.fix_morning_basics_challenges() TO anon;
