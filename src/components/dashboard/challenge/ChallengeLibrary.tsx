...
export function ChallengeLibrary({ 
  ...
  const [selectedCategory, setSelectedCategory] = React.useState<string | null>('mindset');
  const [selectedChallenge, setSelectedChallenge] = React.useState<Challenge | null>(null);
  const [loading, setLoading] = React.useState(true);
  const { challenges: availableChallenges, loading: challengesLoading, error: challengesError } = useChallengeLibrary();
  ...
  const [tier0, setTier0] = React.useState<Challenge | null>(null);
  ...
  React.useEffect(() => {
    ...
    if (expandChallengeId) {
      ...
      const challenge = safeAvailableChallenges.find(c => c.id === expandChallengeId);
      if (challenge) {
  ...
  React.useEffect(() => {
    // For now, use the hardcoded tier0Challenges
    ...
    //     });
    //   } catch (err) {
    //     console.error('Error fetching tier0 challenge:', err);
    //   }
    // };
    // 
    // fetchTier0();
  ...
  const hasCompletedTier1 = React.useMemo(() => {
    ...
    const categoryChallenges = safeAvailableChallenges.filter(c => 
  ...
  const filteredChallenges = React.useMemo(() => {
    ...
    const categoryMatches = safeAvailableChallenges
      .filter(challenge => challenge.category?.toLowerCase() === selectedCategory.toLowerCase())
      .map(challenge => ({
        ...challenge,
        status: 'available'
      }));
  ...
  // Calculate non-premium active challenges count - fix the undefined challenges variable
  const nonPremiumChallengesCount = currentChallenges.filter(c => 
    !safeAvailableChallenges.find(ch => ch.id === c.challenge_id)?.isPremium &&
    !c.challenge_id.startsWith('custom-') // Don't count custom challenges
  ).length;

  const handleStartChallenge = async (challengeId: string) => {
    try {
      // Check if challenge is already active
      if (currentChallenges.some(c => c.challenge_id === challengeId || c.id === challengeId) || 
          (!hasCompletedTier0 && challengeId !== 'mb0' && !challengeId.startsWith('cn_') && !challengeId.startsWith('tc_'))) {
        return;
      }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {hasCompletedTier0 && (
            <div className="text-center mb-4 bg-orange-500/10 border border-orange-500/20 rounded-lg p-1.5">
              <h3 className="text-sm font-medium text-white">Tier 1 Challenges Unlocked!</h3>
            </div>
          )}
                  ...
                  <div className="flex-1 relative">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">+{tier0.fuelPoints} FP</span>
                      </div>
                    </div>
            ...
            <>
              {filteredChallenges.length > 0 ? (
                filteredChallenges.map(challenge => (
                  ...
                  >
                    {renderChallenge(challenge, completedChallenges.includes(challenge.id))}
                  </div>
                ))
              ) : (
          ...
          {!selectedCategory && !hasCompletedTier0 && (
            <div className="mb-6">
              <div className="text-center mb-4 bg-orange-500/5 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-400">Complete Morning Basics Challenge to unlock all Tier 1 Challenges</p>
              </div>
              
              {/* Custom Challenge Info */}
              {customChallengeCount > 0 && (
                <div className="text-center text-gray-300 py-4 bg-gray-700/30 rounded-lg mb-4">
                  You have {customChallengeCount} active custom {customChallengeCount === 1 ? 'challenge' : 'challenges'} (doesn't count toward your limit of {maxChallenges} regular challenges)
                </div>
              )}
            </div>
          )}
      ...
      {selectedChallenge && (
        <ChallengeDetails
          ...
          onStart={() => {
            ...
            if (selectedChallenge.id === 'mb0' || selectedChallenge.challenge_id === 'mb0') {
              navigate(`/challenge/mb0`);
            } else {
              handleStartChallenge(selectedChallenge.id || selectedChallenge.challenge_id);
            }
            onClose();
          }}
        />
            ...
            <div className="flex-1 relative">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {challenge.expertReference && (
                  <div className="flex items-center gap-1 text-xs text-orange-500">
                    <Award size={12} />
                    <span>{typeof challenge.expertReference === 'string' 
                      ? challenge.expertReference.split(' - ')[0] 
                      : challenge.expertReference}</span>
                  </div>
                )}
                {isCompleted && (
                  <span className="text-xs bg-lime-500/20 px-2 py-0.5 rounded text-lime-500">Completed</span>
