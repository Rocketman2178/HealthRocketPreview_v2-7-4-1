...
import { Target, ChevronRight, ChevronLeft, Plus, Trash, Award, Zap, Radio, X, Info, Loader2 } from 'lucide-react';
import { useCustomChallenge } from '../../../hooks/useCustomChallenge';
import { useSupabase } from '../../../contexts/SupabaseContext';
import type { CustomChallengeFormAction, CustomChallengeFormData } from '../../../types/customChallenge';
import { CustomChallengeActionInput } from './CustomChallengeActionInput';

const CATEGORIES = ['Mindset', 'Sleep', 'Exercise', 'Nutrition', 'Biohacking'] as const;
const MIN_ACTIONS = 3;
const MAX_ACTIONS = 12;

const genericGuidance = "Focus on consistency with your custom actions. Start with the easiest actions to build momentum. Track your progress daily and celebrate small wins. Remember that this challenge is designed specifically for your needs and lifestyle. Stay committed to your 21 completions goal!";
...
export function CustomChallengeCreator() {
  ...
  const [step, setStep] = useState<'intro' | 'actions' | 'guidance' | 'review'>('intro');
  ...
  const handleActionChange = (index: number, field: keyof CustomChallengeFormAction, value: string) => {
    const newActions = [...formData.actions];
    newActions[index] = {
      ...newActions[index],
      [field]: value
  ...
  const handleNextStep = async () => {
    if (step === 'intro') {
      console.log('Moving from intro to actions step');
      setStep('actions');
    } else if (step === 'actions') {
      if (!validateActions()) return;
      
      console.log('Moving from actions to review step');
      // Skip guidance step and go directly to review
      setStep('review');
    } else if (step === 'guidance') {
      console.log('Moving from guidance to review step');
      setStep('review');
    }
  };
  ...
  const handlePrevStep = () => {
    if (step === 'actions') {
      console.log('Moving back from actions to intro step');
      setStep('intro');
    } else if (step === 'guidance') {
      console.log('Moving back from guidance to actions step');
      setStep('actions');
    } else if (step === 'review') {
      console.log('Moving back from review to actions step');
      setStep(useAiGuidance ? 'guidance' : 'actions');
    }
  };

  const handleSubmit = async () => {
    console.log("Submitting custom challenge form");
    if (!user?.id) return;
    ...
    try {
      ...
      const cleanFormData = {
        ...
        daily_minimum: Math.min(formData.daily_minimum, formData.actions.length),
      ...
      console.log('Submitting clean form data to createCustomChallenge:', JSON.stringify(cleanFormData, null, 2));
      ...
      <div className="w-full max-w-2xl bg-gray-800 rounded-lg shadow-xl my-8">
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-white">Create Custom Challenge</h2>
          </div>
          <button
            onClick={() => navigate('/')}
        ...
        <div className="p-4 border-b border-gray-700">
          <div className="flex justify-between">
            <div className={`flex flex-col items-center ${step === 'intro' ? 'text-orange-500' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'intro' ? 'bg-orange-500 text-white' : 'bg-gray-700'}`}>
                1
              </div>
              <span className="text-xs mt-1">Intro</span>
            </div>
            <div className={`flex flex-col items-center ${step === 'actions' ? 'text-orange-500' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'actions' ? 'bg-orange-500 text-white' : 'bg-gray-700'}`}>
                2
              </div>
              <span className="text-xs mt-1">Actions</span>
            </div>
            {useAiGuidance && (
              <div className={`flex flex-col items-center ${step === 'guidance' ? 'text-orange-500' : 'text-gray-400'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'guidance' ? 'bg-orange-500 text-white' : 'bg-gray-700'}`}>
                  3
                </div>
                <span className="text-xs mt-1">Guidance</span>
              </div>
            )}
            <div className={`flex flex-col items-center ${step === 'review' ? 'text-orange-500' : 'text-gray-400'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'review' ? 'bg-orange-500 text-white' : 'bg-gray-700'}`}>
                {useAiGuidance ? 4 : 3}
              </div>
        ...
        <div className="p-6">
          {step === 'intro' && (
            <div className="space-y-6">
              <div className="text-center">
                <h3 className="text-xl font-bold text-white mb-2">Create Your Custom Challenge</h3>
              ...
              <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-orange-500 font-medium">+{fpCompletionReward} FP</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-300">Daily Actions:</span>
                    <span className="text-orange-500 font-medium">+{fpDailyReward} FP</span>
                  </div>
                  <div className="flex justify-between items-center">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-300">Previous Custom Challenges:</span>
                    <span className="text-white font-medium">{completionCount}</span>
                  </div>
                  <div className="flex justify-between items-center mt-2 pt-2 border-t border-orange-500/20">
                    ...
                    <span className="text-white">Doesn't count toward your 2-challenge limit</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-700/50 rounded-lg p-4">
                ...
                <ul className="space-y-2">
                  <li className="flex items-start gap-2 text-gray-300">
                    ...
                    <span>Create 3-12 daily actions across health categories</span>
                  </li>
                  <li className="flex items-start gap-2 text-gray-300">
                    <div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-xs text-white">2</span>
                    </div>
                    <span>Set your daily minimum (how many actions to complete each day)</span>
                  </li>
                  <li className="flex items-start gap-2 text-gray-300">
                    <div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-xs text-white">3</span>
                    </div>
                    <span>Complete your minimum daily actions 21 times to finish</span>
                  </li>
                  <li className="flex items-start gap-2 text-gray-300">
                    <div className="w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center flex-shrink-0 mt-0.5">
              ...
              <div className="text-center">
                <p className="text-sm text-gray-400 mb-2">
                  Each completed challenge doubles your FP rewards for the next one!
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">
                  Challenge Name
                  <span className="text-xs text-orange-500 ml-2">* All fields required</span>
                </label>
                <input
                  type="text"
                  value={formData.name}
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <input
                    ...
                    onChange={(e) => handleDailyMinimumChange(parseInt(e.target.value))}
                    className="flex-1"
                  />
              ...
              <div className="bg-gray-700/50 rounded-lg p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-sm text-gray-300">
                    Your challenge will include generic expert guidance to help you succeed.
                  </div>
                </div>
              </div>
            </div>
          ...
          {step === 'review' && (
            <div className="space-y-6">
              <div className="text-center">
                <h3 className="text-xl font-bold text-white mb-2">Review Your Challenge</h3>
                <p className="text-gray-300">
              ...
              <div className="bg-gray-700/50 rounded-lg p-4">
                <h4 className="text-lg font-medium text-white mb-3">{formData.name}</h4>
                <div className="flex justify-between items-center mb-4">
                  <div className="text-sm text-gray-300">
                    Daily Minimum: <span className="text-white font-medium">{formData.daily_minimum}</span>
                  </div>
                  <div className="text-sm text-gray-300">
                    Total Actions: <span className="text-white font-medium">{formData.actions.length}</span>
                  </div>
                </div>
                  ...
                  {formData.actions.map((action, index) => (
                    <div key={index} className="bg-gray-800/50 rounded-lg p-3">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-white font-medium">{action.action_text}</span>
                      </div>
                      <div className="ml-8">
                        <div className="text-xs text-gray-400 mb-1">{action.description}</div>
                        <div className="text-xs text-orange-500">{action.category}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-gray-700/50 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-white font-medium mb-2">Expert Guidance</h4>
