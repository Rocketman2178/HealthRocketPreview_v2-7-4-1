...
export function useCustomChallenge(userId: string | undefined) {
  ...
  const [dailyCompletions, setDailyCompletions] = useState<CustomChallengeDailyCompletion[]>([]);
  ...
  const checkCanCreate = async () => {
    ...
    console.log("Checking if user can create custom challenge");
    try {
      setLoading(true);
      setError(null);

      const result = await CustomChallengeService.canCreateCustomChallenge(userId);

      if (result.error) {
        setError(result.error);
        console.error("Error checking if user can create custom challenge:", result.error);
        return;
      }
  ...
  const fetchUserChallenge = async () => {
    ...
    console.log("Fetching user custom challenge for userId:", userId);

    try {
      setLoading(true);
      setError(null);

      const result = await CustomChallengeService.getUserCustomChallenge(userId);
      console.log("getUserCustomChallenge result:", result);

      if (result.error) {
        setError(result.error);
        console.error("Error fetching user custom challenge:", result.error);
        return;
      }

      setHasActiveChallenge(result.hasActiveChallenge);
      console.log("Has active challenge:", result.hasActiveChallenge);

      if (result.hasActiveChallenge && result.challenge) {
        console.log("Setting active challenge:", result.challenge);
      ...
      console.error("Error in fetchUserChallenge:", err);
      setError(err instanceof Error ? err.message : 'Unknown error');
    } finally {
      setLoading(false);
    }
  ...
  const createCustomChallenge = async (formData: CustomChallengeFormData) => {
    ...
    console.log("Creating custom challenge with formData:", JSON.stringify(formData, null, 2));
    ...
    try {
      ...
      console.log("Calling CustomChallengeService.createCustomChallenge with validated data");
      ...
      if (!result.success) {
        ...
        console.error("Failed to create custom challenge:", result.error);
        console.error("Failed to create custom challenge:", result.error);
      ...
      console.log("Custom challenge created successfully:", result);
  ...
  const completeDailyActions = async (challengeId: string, completedActions: { action_id: string }[]) => {
  ...
  const cancelCustomChallenge = async (challengeId: string) => {
    ...
    try {
      ...
      const result = await CustomChallengeService.cancelCustomChallenge(userId, challengeId);
  ...
  const generateExpertGuidance = useCallback(async (actions: { action_text: string; category: string }[]) => {
  ...
  useEffect(() => {
    if (userId) {
      console.log("Initializing custom challenge data for userId:", userId);
      Promise.all([
        fetchUserChallenge(),
        checkCanCreate()
      ]).then(() => {
        console.log("Custom challenge initialization complete");
      }).catch(err => {
